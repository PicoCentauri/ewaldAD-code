...
def prepare_model(datamodule: LitDataModule, parameters: dict) -> BPPSLodeModel:
    ...
    model = BPPSLodeModel(unique_elements=datamodule.unique_elements, **parameters["model"])
...
def prepare_trainer(logger: pl.loggers.WandbLogger, parameters: dict) -> pl.Trainer:
    ...
    trainer = pl.Trainer(callbacks=callbacks, logger=logger, **parameters["trainer"], inference_mode=False)
    return trainer
